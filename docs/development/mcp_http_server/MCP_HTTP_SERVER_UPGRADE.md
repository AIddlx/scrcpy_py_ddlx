...
> 日期：2026-02-16 创建，2026-03-02 更新
> 目标：升级 scrcpy_http_mcp_server.py 以支持网络模式和更多参数
> 状态：**阶段 1-5 已完成**

## 已完成

### 阶段 5：命令行参数重构 ✅ (2026-03-02)

**详见**: [STARTUP_FLOW_REFACTOR.md](STARTUP_FLOW_REFACTOR.md)

- [x] 新增 `--force` 参数：强制推送驻留服务端（终止旧的，推送新的）
- [x] 新增 `--stop-server` 参数：仅终止驻留服务端
- [x] 参数组合验证：检测无效组合并给出明确错误提示
- [x] 启动流程重构：`on_startup` 函数重新设计网络模式分支
- [x] Windows 控制台兼容：替换 Unicode 字符为 ASCII
- [x] ADB 模式组件信息增强：显示实际编码器和请求参数
