...
**症状**: `record_audio` 实际录制时长少于设定的 duration

**原因**:
- 录音器只能录制**设备实际播放的音频**
- 如果设备静默或延迟播放，音频数据流会有间隙
- 最终时长基于实际收到的音频帧计算（`frames / sample_rate`）

**示例日志**:
```
Max duration: 5 sec
Duration: 4.98 sec  # 实际录制时长
Frames: 239040
```

**说明**: 这是预期行为。scrcpy 只能录制设备正在播放的音频，无法录制静默期。

**待改进**（低优先级）:
- 返回实际录制时长而非请求时长
- 添加警告提示（当实际时长与请求时长差异较大时）
